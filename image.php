...
$thumbnail_prefix = $conf['thumbnail_prefix'];
$media_path = $conf['system_pic_path'];
...
function __g($key) {
 ...
 function avs_decrypt($string) {
    global $conf;
    $output = false;

    $encrypt_method = "AES-256-CBC";
    $secret_key = "vldthemes_blursecret";
    $secret_iv = "ask9124%CaSDdsaY335u9§=";
...
if(! defined('NOT_FOUND_IMAGE') )			define ('NOT_FOUND_IMAGE', 'error_image.jpg');							// Image to serve if any 404 occurs 
if(! defined('ERROR_IMAGE') )				define ('ERROR_IMAGE', 'error_image.jpg');								// Image to serve if an error occurs instead of showing error message 
...
if(! defined('DEFAULT_F') )					define ('DEFAULT_F', 3);								// Default image filters. Allows overrid in timthumb-config.php
...
if(! defined('DEFAULT_WIDTH') )				define ('DEFAULT_WIDTH', 300);							// Default thumbnail width. Allows overrid in timthumb-config.php
if(! defined('DEFAULT_HEIGHT') )			define ('DEFAULT_HEIGHT', 300);							// Default thumbnail height. Allows overrid in timthumb-config.php
...
class timthumb {
	...
	public function __construct(){
		...
		$this->myHost = preg_replace('/^www\./i', '', $_SERVER['HTTP_HOST']);
                ...
                global $thumbnail_prefix,$media_path;
                
                if(__g('i') == false) {
                     $this->src = str_replace('/'.$thumbnail_prefix,'/',$this->param('src'));
                     $this->src = $media_path.$this->src;
                }
                else {
                   $this->src = $media_path.__g('p').avs_decrypt(__g('i')); 
                }
